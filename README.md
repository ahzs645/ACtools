...
- `popup` as a toolkit drawer / app launcher
...
- `Ramona Day View` overlay, ported from Walter's page script into a jQuery-free content feature
- `Availability Test` workspace in the popup that posts a single-day availability entry through the current AlayaCare browser session
...
- The build uses `Vite` for the popup and `esbuild` for `background.js` and `content.js`.
